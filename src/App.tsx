...
export default function App(){
 ...
 function snapshot(label:string,lines=p.lines,sticks=p.sticks,production=p.productionPlan){return{id:crypto.randomUUID(),createdAt:Date.now(),label,lines:cloneLines(lines),sticks:sticks.map(s=>({...s})),constraints:(p.constraints??[]).map(c=>({...c})),productionPlan:production?JSON.parse(JSON.stringify(production)):undefined}}
...
function restoreVersion(id:string){const v=p.history.find(x=>x.id===id);if(!v)return;setP(q=>({...q,lines:cloneLines(v.lines),sticks:v.sticks?.map(s=>({...s}))??q.sticks,constraints:v.constraints?.map(c=>({...c}))??q.constraints,connections:findConnectionCandidates(v.lines,q.settings.connectDistance),productionPlan:v.productionPlan?JSON.parse(JSON.stringify(v.productionPlan)):undefined,history:[...q.history,snapshot('Before restore',q.lines,q.sticks,q.productionPlan)],updatedAt:Date.now()}));setProductionPlan(v.productionPlan?JSON.parse(JSON.stringify(v.productionPlan)):null);setProductionSelected(0)}
