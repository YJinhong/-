...
export default function App(){
 ...
 useEffect(()=>{if(!projectHydrated)return;const t=window.setTimeout(()=>saveProject(p),250);setProjects(x=>[p,...x.filter(a=>a.id!==p.id)]);return()=>clearTimeout(t)},[p,projectHydrated]); useEffect(()=>{if(!productionHydrated)return;setP(q=>{const same=(q.productionPlan?.id??null)===(productionPlan?.id??null)&&JSON.stringify(q.productionPlan?.steps??[])===JSON.stringify(productionPlan?.steps??[]);return same?q:{...q,productionPlan:productionPlan??undefined,updatedAt:Date.now()}})},[productionPlan,productionHydrated]);
