...
export default function App(){
 ...
 useEffect(()=>{const c=canvas.current;if(!c)return;const dpr=window.devicePixelRatio||1,w=c.clientWidth,h=c.clientHeight;c.width=w*dpr;c.height=h*dpr;const x=c.getContext('2d')!;x.setTransform(dpr,0,0,dpr,0,0);x.fillStyle=dark?'#101216':'#f4f5f7';x.fillRect(0,0,w,h);x.save();x.translate(w/2+pan.x,h/2+pan.y);x.scale(zoom,zoom);
  ...
  if(productionPreview&&productionPlan&&productionPlan.steps[productionSelected]&&!playing){const step=productionPlan.steps[productionSelected];const target=p.lines.find(l=>l.id===step.lineId);if(target){x.save();x.globalAlpha=.22;x.strokeStyle='#999';x.lineWidth=2;for(const l of p.lines){if(l.id!==target.id)drawLine(l,'#999',1)}x.globalAlpha=1;drawLine(target,'#6d5dfc',Math.max(3,target.width*2));const sxp=target.points[0].x*sx,syp=target.points[0].y*sy,exp=target.points.at(-1)!.x*sx,eyp=target.points.at(-1)!.y*sy;x.fillStyle='#35d07f';x.beginPath();x.arc(sxp,syp,7,0,Math.PI*2);x.fill();x.fillStyle='#ef5350';x.beginPath();x.arc(exp,eyp,7,0,Math.PI*2);x.fill();x.restore()}}
...
function rebuildTopology(){setP(q=>{const graph=rebuildGraph(q.lines),connections=findConnectionCandidates(q.lines,q.settings.connectDistance),warnings=[...analyze(q.lines),...graph.filter(n=>n.degree>=3).map(n=>({id:crypto.randomUUID(),severity:'low' as const,message:'Junction node detected; review the joint before cooking.',lineIds:n.lineIds}))];return{...q,connections,warnings,sticks:recommendSticks(q.lines),updatedAt:Date.now()}})}
 function buildProductionPlan(project:Project):ProductionPlan{const path=minimumPenLiftPath(project.lines).order;let total=0;const steps=path.map((l,i)=>{const start=l.points[0],end=l.points.at(-1)!;const lengthMm=Math.round(polylineLengthMm(l.points,project.widthMm,project.heightMm)*10)/10;total+=lengthMm;const attached=project.sticks.find(s=>Math.hypot(s.x-start.x,s.y-start.y)<.06);return{id:crypto.randomUUID(),index:i+1,lineId:l.id,action:(l.closed?'CLOSE':attached?'SUPPORT':i===0?'DRAW':'LIFT') as 'DRAW'|'LIFT'|'CLOSE'|'SUPPORT',lengthMm,start,end,penLiftBefore:i>0,supportStickId:attached?.id,note:l.closed?'Close this contour smoothly.':attached?'Place/check support before continuing this span.':i===0?'Start from this endpoint and maintain a steady syrup flow.':'Lift cleanly, reposition to the next start point, then continue.'}});const lifts=Math.max(0,path.length-1);return{id:crypto.randomUUID(),createdAt:Date.now(),estimatedPenLifts:lifts,totalLengthMm:Math.round(total*10)/10,steps,summary:[path.length+' drawing segments',lifts+' estimated pen lifts',Math.round(total*10)/10+' mm total path',project.sticks.length+' support sticks',project.warnings.filter(w=>w.severity==='high').length+' high-risk warnings']}}\nfunction focusProductionStep(step:ProductionPlan['steps'][number]){const idx=p.lines.findIndex(l=>l.id===step.lineId);setProductionSelected(Math.max(0,step.index-1));if(idx>=0){setSelectedLine(step.lineId);setSelectedLines([step.lineId]);setViewMode('result')}}
 function rebuildProductionPlan(){setProductionPlan(buildProductionPlan(p));setProductionSelected(0)}
 function moveProductionStep(from:number,to:number){setProductionPlan(q=>{if(!q||to<0||to>=q.steps.length)return q;const steps=[...q.steps];const moved=steps.splice(from,1)[0];if(!moved)return q;steps.splice(to,0,moved);return{...q,steps:steps.map((s,i)=>({...s,index:i+1,penLiftBefore:i>0}) )}})}
 function deleteProductionStep(index:number){setProductionPlan(q=>{if(!q)return q;const steps=q.steps.filter((_,i)=>i!==index).map((s,i)=>({...s,index:i+1,penLiftBefore:i>0}));return{...q,steps,estimatedPenLifts:Math.max(0,steps.length-1)}})}
