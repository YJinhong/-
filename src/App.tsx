...
export default function App(){
 ...
 useEffect(()=>{if(!p.imageData){setOriginalImage(null);return}const img=new Image();img.decoding='async';img.onload=()=>setOriginalImage(img);img.onerror=()=>setOriginalImage(null);img.src=p.imageData},[p.imageData]); useEffect(()=>{const redraw=()=>setViewportTick(v=>v+1);window.addEventListener('resize',redraw);window.addEventListener('orientationchange',redraw);window.visualViewport?.addEventListener('resize',redraw);return()=>{window.removeEventListener('resize',redraw);window.removeEventListener('orientationchange',redraw);window.visualViewport?.removeEventListener('resize',redraw)}},[]);
 useEffect(()=>{if(!projectHydrated)return;const t=window.setTimeout(()=>saveProject(p),250);setProjects(x=>[p,...x.filter(a=>a.id!==p.id)]);return()=>clearTimeout(t)},[p,projectHydrated]); useEffect(()=>{if(!productionHydrated)return;setP(q=>{const same=JSON.stringify(q.productionPlan??null)===JSON.stringify(productionPlan??null);return same?q:{...q,productionPlan:productionPlan??undefined,updatedAt:Date.now()}})},[productionPlan,productionHydrated]);
...
for(let i=0;i<files.length;i++){
  const r=results[i];if(session!==batchSession.current){processor.dispose();if(batchProcessor.current===processor)batchProcessor.current=null;return}if(!r)continue;
...
function rebuildTopology(){setP(q=>{const graph=rebuildGraph(q.lines),connections=findConnectionCandidates(q.lines,q.settings.connectDistance),warnings=[...analyze(q.lines),...graph.filter(n=>n.degree>=3).map(n=>({id:crypto.randomUUID(),severity:'low' as const,message:'Junction node detected; review the joint before cooking.',lineIds:n.lineIds}))];return{...q,connections,warnings,sticks:recommendSticks(q.lines),updatedAt:Date.now()}})}
 ...
 function rejectConnection(id:string){setP(q=>{const target=q.connections.find(c=>c.id===id);if(!target||target.status==='rejected')return q;return{...q,connections:q.connections.map(c=>c.id===id?{...c,status:'rejected' as const}:c),history:[...q.history,snapshot('Reject connection',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}})}
 function acceptAll(){setP(q=>{if(!q.connections.some(c=>c.status==='pending'))return q;return{...q,connections:q.connections.map(c=>c.status==='pending'?{...c,status:'accepted' as const}:c),history:[...q.history,snapshot('Accept all connections',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}})}
 function applyAccepted(){setP(q=>{const before=cloneLines(q.lines),lines=applyConnections(q.lines,q.connections);const ordered=minimumPenLiftPath(lines).order,connections=findConnectionCandidates(ordered,q.settings.connectDistance).filter(c=>!q.connections.some(old=>old.a===c.a&&old.b===c.b&&old.status==='rejected'));return{...q,lines:ordered,productionPlan:undefined,connections,warnings:analyze(ordered),sticks:recommendSticks(ordered),history:[...q.history,snapshot('Apply accepted connections',before,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}})}
 function undo(){
  const h=p.history.at(-1);if(!h)return;
  historyNavigation.current=true;
  setRedoStack(r=>[...r,snapshot('Redo',p.lines,p.sticks,p.productionPlan,p.constraints,p.connections)]);
  setP(q=>({...q,lines:cloneLines(h.lines),sticks:h.sticks?.map(s=>({...s}))??q.sticks,constraints:h.constraints?.map(c=>({...c}))??q.constraints,connections:h.connections?.map(c=>({...c}))??findConnectionCandidates(h.lines,q.settings.connectDistance),warnings:analyze(h.lines),productionPlan:h.productionPlan,history:q.history.slice(0,-1),updatedAt:Date.now()}));setProductionPlan(h.productionPlan?JSON.parse(JSON.stringify(h.productionPlan)):null);setProductionSelected(0);
}
function redo(){
  const h=redoStack.at(-1);if(!h)return;
  historyNavigation.current=true;
  setRedoStack(r=>r.slice(0,-1));
  setP(q=>({...q,lines:cloneLines(h.lines),sticks:h.sticks?.map(s=>({...s}))??q.sticks,constraints:h.constraints?.map(c=>({...c}))??q.constraints,connections:h.connections?.map(c=>({...c}))??findConnectionCandidates(h.lines,q.settings.connectDistance),warnings:analyze(h.lines),productionPlan:h.productionPlan,history:[...q.history,snapshot('Undo',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}));setProductionPlan(h.productionPlan?JSON.parse(JSON.stringify(h.productionPlan)):null);setProductionSelected(0);
}
 ...
 function transformSelected(mode:'left'|'right'|'top'|'bottom'|'centerX'|'centerY'|'distributeX'|'distributeY'|'scaleUp'|'scaleDown'){
  ...
  setP(q=>({...q,lines:solved,productionPlan:undefined,connections:findConnectionCandidates(solved,q.settings.connectDistance),warnings:analyze(solved),sticks:recommendSticks(solved),history:[...q.history,snapshot('Group '+mode,q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}));
 }
 function updateConstraintValue(id:string,value:number,value2?:number){
  if(!Number.isFinite(value))return;
  setP(q=>{const constraints=(q.constraints??[]).map(c=>c.id===id?{...c,value,value2:c.type==='fixedPoint'?value2??c.value2:c.value2}:c);const lines=solveConstraints(q.lines,constraints,q.widthMm,q.heightMm,8).lines;return{...q,constraints,lines,connections:findConnectionCandidates(lines,q.settings.connectDistance),warnings:analyze(lines),history:[...q.history,snapshot('Edit CAD constraint',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}});
 }
 ...
 function removeConstraint(id:string){setP(q=>{const removed=(q.constraints??[]).find(c=>c.id===id);if(!removed)return q;const constraints=(q.constraints??[]).filter(c=>c.id!==id);const lines=q.lines.map(l=>{if(l.id!==removed.lineId&&l.id!==removed.referenceLineId)return l;return applyActiveConstraints(l,constraints,q.widthMm,q.heightMm,q.lines)});return{...q,lines,constraints,connections:findConnectionCandidates(lines,q.settings.connectDistance),warnings:analyze(lines),sticks:recommendSticks(lines),history:[...q.history,snapshot('Remove CAD '+removed.type,q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}})}
 ...
 function addFixedPoint(){
  ...
  setP(q=>({...q,constraints:[...(q.constraints??[]).filter(c=>!(c.lineId===l.id&&c.type==='fixedPoint')),{id:crypto.randomUUID(),lineId:l.id,type:'fixedPoint',value:a.x*p.widthMm,value2:a.y*p.heightMm,createdAt:now}],history:[...q.history,snapshot('CAD fixed point',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:now}))
 }
 ...
 function splitSelected(){if(!selectedLine)return;const l=p.lines.find(x=>x.id===selectedLine);if(!l||l.points.length<4)return;const mid=Math.floor(l.points.length/2);const a={...l,id:crypto.randomUUID(),points:l.points.slice(0,mid+1)},b={...l,id:crypto.randomUUID(),points:l.points.slice(mid)};setP(q=>({...q,lines:[...q.lines.filter(x=>x.id!==selectedLine),a,b],productionPlan:undefined,connections:[],constraints:(q.constraints??[]).filter(c=>c.lineId!==selectedLine&&c.referenceLineId!==selectedLine),history:[...q.history,snapshot('Split line',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}));setSelectedLine(a.id);setSelectedLines([a.id]);setRelationLine(null)}
function mergeSelected(){if(!selectedLine)return;const base=p.lines.find(x=>x.id===selectedLine);if(!base)return;let other:Line|undefined,reverseBase=false,reverseOther=false,best=Infinity;for(const candidate of p.lines){if(candidate.id===base.id)continue;const pairs=[[0,candidate.points.length-1],[candidate.points.length-1,0]] as const;for(const [ai,bi] of pairs){const d=Math.hypot(base.points.at(-1)!.x-candidate.points[ai].x,base.points.at(-1)!.y-candidate.points[ai].y);if(d<p.settings.connectDistance&&d<best){best=d;other=candidate;reverseOther=ai===candidate.points.length-1}}}if(!other)return;const bp=reverseBase?[...base.points].reverse():[...base.points],op=reverseOther?[...other.points].reverse():[...other.points];const merged={...base,id:crypto.randomUUID(),points:[...bp,...op]};setP(q=>({...q,lines:[...q.lines.filter(x=>x.id!==base.id&&x.id!==other!.id),merged],productionPlan:undefined,connections:[],constraints:(q.constraints??[]).filter(c=>c.lineId!==base.id&&c.lineId!==other!.id&&c.referenceLineId!==base.id&&c.referenceLineId!==other!.id),history:[...q.history,snapshot('Merge lines',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}));setSelectedLine(merged.id);setSelectedLines([merged.id]);setRelationLine(null)}
...
function updateStick(id:string,patch:Partial<NonNullable<Project['sticks']>[number]>){setP(q=>({...q,sticks:q.sticks.map(s=>{if(s.id!==id)return s;const n={...s,...patch},check=stickCollision(n,q.lines);let nearest=Infinity;for(const l of q.lines)for(const pt of l.points)nearest=Math.min(nearest,Math.hypot(n.x-pt.x,n.y-pt.y));const geometry=Math.max(0,Math.min(100,70-Math.min(45,nearest*120)+Math.min(20,n.length/4)+(n.kind==='avoid'?-20:0)-(check.collision?25:0)));return{...n,score:Math.round(geometry)}}),history:[...q.history,snapshot('Edit support stick',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}))}
 function addStick(){const s={id:crypto.randomUUID(),x:0,y:0,length:40,angle:90,score:50,kind:'optional' as const};setP(q=>({...q,sticks:[...q.sticks,s],history:[...q.history,snapshot('Add support stick',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}));setSelectedStick(s.id)}
 function deleteStick(){if(!selectedStick)return;setP(q=>{const productionPlan=q.productionPlan?{...JSON.parse(JSON.stringify(q.productionPlan)),steps:q.productionPlan.steps.map(step=>step.supportStickId===selectedStick?{...step,supportStickId:undefined}:step)}:q.productionPlan;return{...q,sticks:q.sticks.filter(s=>s.id!==selectedStick),productionPlan,history:[...q.history,snapshot('Delete support stick',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}});setProductionPlan(q=>q?{...JSON.parse(JSON.stringify(q)),steps:q.steps.map(step=>step.supportStickId===selectedStick?{...step,supportStickId:undefined}:step)}:q);setSelectedStick(null)}
 ...
 if(cursor==='connect'){const id=hitLine(pt);if(id){if(!connectStart)setConnectStart(id);else if(connectStart!==id){setP(q=>{const aa=q.lines.find(l=>l.id===connectStart),bb=q.lines.find(l=>l.id===id);if(!aa||!bb)return q;const cc=findConnectionCandidates([aa,bb],q.settings.connectDistance)[0];return cc?{...q,connections:[...q.connections,cc],history:[...q.history,snapshot('Add connection',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}:q});setConnectStart(null)}}return}
 if(cursor==='draw')setP(q=>({...q,lines:[...q.lines,{id:crypto.randomUUID(),points:[pt,{x:pt.x+.04,y:pt.y+.04}],width:q.settings.minWidthMm}],productionPlan:undefined,connections:[],history:[...q.history,snapshot('Draw',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}))}

 function movePoint(ev:ReactPointerEvent<HTMLCanvasElement>){if(!panDragging){const cp=canvasPoint(ev);setCursorPoint(cp);if(cursor==='select')setDiagnosticHover(diagnosticHit(cp));}if(panDragging){setPan({x:ev.clientX-panStart.x,y:ev.clientY-panStart.y});return}const raw=canvasPoint(ev),q=snapPoint(raw),dx=q.x-dragStart.x,dy=q.y-dragStart.y;if(cursor==='measure'&&measureStart){setMeasureEnd(raw);return;}if(selectionBox){setSelectionBox(b=>b?{...b,end:raw}:null);return;}if(stickDragging&&selectedStick){setDragStart(q);setP(v=>({...v,sticks:v.sticks.map(s=>s.id===selectedStick?{...s,x:s.x+dx,y:s.y+dy}:s),updatedAt:Date.now()}));return;}if(nodeDragging&&selectedNode){setDragStart(q);setP(v=>{const lines=v.lines.map(l=>{if(l.id!==selectedNode.lineId)return l;const moved={...l,points:l.points.map((pt,i)=>i===selectedNode.index?{x:q.x,y:q.y}:pt)};return applyActiveConstraints(moved,v.constraints??[],v.widthMm,v.heightMm,v.lines)});return{...v,lines,productionPlan:undefined,connections:findConnectionCandidates(lines,v.settings.connectDistance),warnings:analyze(lines),sticks:recommendSticks(lines),updatedAt:Date.now()}});return}if(!dragging||!selectedLines.length)return;setDragStart(q);setP(v=>{const lines=v.lines.map(l=>{if(!selectedLines.includes(l.id))return l;const moved={...l,points:l.points.map(pt=>({x:pt.x+dx,y:pt.y+dy}))};return applyActiveConstraints(moved,v.constraints??[],v.widthMm,v.heightMm,v.lines)});return{...v,lines,productionPlan:undefined,connections:findConnectionCandidates(lines,v.settings.connectDistance),warnings:analyze(lines),sticks:recommendSticks(lines),updatedAt:Date.now()}})}
 function endPoint(ev?:ReactPointerEvent<HTMLCanvasElement>){setPanDragging(false);if(cursor==='measure'&&measureStart&&ev){setMeasureEnd(canvasPoint(ev));return}if(selectionBox){const a=selectionBox.start,b=selectionBox.end;const minX=Math.min(a.x,b.x),maxX=Math.max(a.x,b.x),minY=Math.min(a.y,b.y),maxY=Math.max(a.y,b.y);const ids=p.lines.filter(l=>l.points.some(q=>q.x>=minX&&q.x<=maxX&&q.y>=minY&&q.y<=maxY)).map(l=>l.id);setSelectedLines(ids);setSelectedLine(ids.at(-1)??null);setRelationLine(ids.length>=2?ids.at(-2)??null:null);setSelectionBox(null);setDragging(false);setNodeDragging(false);setStickDragging(false);dragHistory.current=null;return}const hadDrag=dragHistory.current;const wasDragging=dragging||nodeDragging||stickDragging||graphDragging;setDragging(false);setNodeDragging(false);setStickDragging(false);setGraphDragging(false);if(hadDrag&&wasDragging){setP(q=>{const changed=JSON.stringify({lines:q.lines,sticks:q.sticks})!==JSON.stringify({lines:hadDrag.lines,sticks:hadDrag.sticks});return changed?{...q,history:[...q.history,hadDrag],updatedAt:Date.now()}:q});}dragHistory.current=null;if(!snap)return;setP(q=>{const lines=q.lines.map(l=>({...l,points:l.points.map(pt=>({...pt}))}));let changed=false;for(let i=0;i<lines.length;i++)for(let j=i+1;j<lines.length;j++){for(const ai of [0,lines[i].points.length-1])for(const bj of [0,lines[j].points.length-1]){const a=lines[i].points[ai],b=lines[j].points[bj];if(Math.hypot(a.x-b.x,a.y-b.y)<.012){const m={x:(a.x+b.x)/2,y:(a.y+b.y)/2};lines[i].points[ai]=m;lines[j].points[bj]=m;changed=true}}}if(!changed)return q;const graph=rebuildGraph(lines),warnings=[...analyze(lines),...graph.filter(n=>n.degree>=3).map(n=>({id:crypto.randomUUID(),severity:'low' as const,message:'Junction node detected; review the joint before cooking.',lineIds:n.lineIds}))];return{...q,lines,connections:findConnectionCandidates(lines,q.settings.connectDistance),warnings,sticks:recommendSticks(lines),history:[...q.history,snapshot('Snap and merge endpoints',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}})}

 function exportProjectJson(){const blob=new Blob([projectJson(p)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=p.name+'.sugardraw.json';a.style.display='none';document.body.appendChild(a);a.click();setTimeout(()=>{a.remove();URL.revokeObjectURL(url)},1000)}
async function runDownload(action:()=>void|Promise<void>){
 try{await action();setMenu(null)}catch(e){alert('Export failed: '+(e instanceof Error?e.message:String(e)))}
}
async function importProjectJson(file:File){
 try{
  ...
  setP(imported);setProductionPlan(imported.productionPlan??null);setProductionSelected(0);setSelectedLine(null);setSelectedLines([]);setRelationLine(null);setSelectedNode(null);setSelectedStick(null);setSelectedGraphNode(null);setConnectStart(null);setMeasureStart(null);setMeasureEnd(null);setDiagnosticFocus(null);setDiagnosticHover(null);setPan({x:0,y:0});setZoom(1);
 }catch(e){alert('Unable to import project: '+(e instanceof Error?e.message:String(e)))}
}
function newProject(){const n=blank();setP(n);setProductionPlan(null);setProductionSelected(0);setSelectedLine(null);setSelectedLines([]);setRelationLine(null);setSelectedNode(null);setSelectedStick(null);setSelectedGraphNode(null);setConnectStart(null);setMeasureStart(null);setMeasureEnd(null);setDiagnosticFocus(null);setDiagnosticHover(null);setPan({x:0,y:0});setZoom(1)}
function duplicateProject(){const now=Date.now(),stickMap=new Map<string,string>(),sticks=p.sticks.map(s=>{const id=crypto.randomUUID();stickMap.set(s.id,id);return{...s,id}}),remapPlan=(plan:ProductionPlan|undefined)=>plan?{...JSON.parse(JSON.stringify(plan)),steps:plan.steps.map(s=>({...s,supportStickId:s.supportStickId?(stickMap.get(s.supportStickId)??s.supportStickId):undefined}))}:undefined,n={...p,id:crypto.randomUUID(),name:p.name+' Copy',createdAt:now,updatedAt:now,lines:cloneLines(p.lines),sticks,connections:p.connections.map(c=>({...c,id:crypto.randomUUID()})),history:p.history.map(v=>{const versionStickMap=new Map<string,string>(),versionSticks=v.sticks?.map(s=>{const id=crypto.randomUUID();versionStickMap.set(s.id,id);return{...s,id}});const versionPlan=v.productionPlan?{...JSON.parse(JSON.stringify(v.productionPlan)),steps:v.productionPlan.steps.map(s=>({...s,supportStickId:s.supportStickId?(versionStickMap.get(s.supportStickId)??s.supportStickId):undefined}))}:undefined;return{...v,id:crypto.randomUUID(),lines:cloneLines(v.lines),sticks:versionSticks,connections:v.connections?.map(c=>({...c,id:crypto.randomUUID()})),constraints:v.constraints?.map(c=>({...c})),productionPlan:versionPlan}}),constraints:p.constraints?.map(c=>({...c})),productionPlan:remapPlan(p.productionPlan)};setP(n);setProductionPlan(n.productionPlan??null);setProductionSelected(0);setSelectedLine(null);setSelectedLines([]);setRelationLine(null);setSelectedNode(null);setSelectedStick(null);setSelectedGraphNode(null);setConnectStart(null);setProjects(xs=>[n,...xs.filter(x=>x.id!==n.id)]);saveProject(n)}
function deleteCurrentProject(){if(!confirm('Delete this project?'))return;const id=p.id;deleteProject(id).then(()=>{setProjects(xs=>{const remaining=xs.filter(x=>x.id!==id);const next=remaining[0]??blank();setP(next);setProductionPlan(next.productionPlan??null);setProductionSelected(0);setSelectedLine(null);setSelectedLines([]);setRelationLine(null);setSelectedNode(null);setSelectedStick(null);setSelectedGraphNode(null);setConnectStart(null);setMeasureStart(null);setMeasureEnd(null);setDiagnosticFocus(null);setDiagnosticHover(null);return remaining})})}
...
function restoreVersion(id:string){const v=p.history.find(x=>x.id===id);if(!v)return;setP(q=>({...q,lines:cloneLines(v.lines),sticks:v.sticks?.map(s=>({...s}))??q.sticks,constraints:v.constraints?.map(c=>({...c}))??q.constraints,connections:v.connections?.map(c=>({...c}))??findConnectionCandidates(v.lines,q.settings.connectDistance),warnings:analyze(v.lines),productionPlan:v.productionPlan?JSON.parse(JSON.stringify(v.productionPlan)):undefined,history:[...q.history,snapshot('Before restore',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}));setProductionPlan(v.productionPlan?JSON.parse(JSON.stringify(v.productionPlan)):null);setProductionSelected(0);setSelectedLine(null);setSelectedLines([]);setRelationLine(null);setSelectedNode(null);setSelectedStick(null);setSelectedGraphNode(null);setConnectStart(null);setDiagnosticFocus(null);setDiagnosticHover(null)}
function duplicateVersion(id:string){const v=p.history.find(x=>x.id===id);if(!v)return;const stickMap=new Map<string,string>();const sticks=v.sticks?.map(s=>{const next=crypto.randomUUID();stickMap.set(s.id,next);return{...s,id:next}})??[];const productionPlan=v.productionPlan?{...JSON.parse(JSON.stringify(v.productionPlan)),steps:v.productionPlan.steps.map(s=>({...s,supportStickId:s.supportStickId?(stickMap.get(s.supportStickId)??s.supportStickId):undefined}))}:undefined;const connections=v.connections?.map(c=>({...c,id:crypto.randomUUID()}));setP(q=>({...q,history:[...q.history,{...v,id:crypto.randomUUID(),createdAt:Date.now(),label:v.label+' Copy',lines:cloneLines(v.lines),sticks,constraints:v.constraints?.map(c=>({...c})),connections,productionPlan}],updatedAt:Date.now()}))}
 ...
 <div className="menuWrap"><button onClick={()=>setMenu(menu==='file'?null:'file')}>File</button>{menu==='file'&&<div className="menuPopup">
  ...
  <button onClick={()=>{exportProjectJson();setMenu(null)}}>Export Project JSON</button><button onClick={()=>runDownload(()=>downloadSvg(p))}>Export SVG</button><button onClick={()=>runDownload(()=>downloadPng(p))}>Export PNG</button><button onClick={()=>runDownload(()=>planPdf(p))}>Production Plan PDF</button><button onClick={()=>runDownload(()=>zipProjects([p]))}>Export ZIP</button>
 </div>}</div>
 ...
 <button onClick={undo} disabled={!p.history.length} title="Undo (Ctrl+Z)"><Undo2/></button><button onClick={redo} disabled={!redoStack.length} title="Redo (Ctrl+Y / Ctrl+Shift+Z)"><Redo2/></button><button onClick={async()=>await planPdf(p)} title="Download production drawing"><Download/></button></nav><div className="topActions"><button onClick={()=>setDark(v=>!v)}>{dark?<Sun/>:<Moon/>}</button><button onClick={()=>setBatchOpen(v=>!v)}><Layers/> Batch</button><button className="primary" onClick={()=>document.getElementById('files')?.click()}><Upload/> Import</button><input id="files" hidden type="file" multiple accept=".png,.jpg,.jpeg,image/png,image/jpeg" onChange={e=>importFiles(e.target.files)}/></div></header>
