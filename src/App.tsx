...
export default function App(){
 ...
 useEffect(()=>{listProjects().then(xs=>{const n=xs.map(normalize);setProjects(n);if(n.length){const first=n[0];setP(first);setProductionPlan(first.productionPlan??null);setProductionSelected(0)}setProductionHydrated(true);setProjectHydrated(true)})},[]); useEffect(()=>{const onKey=(e:KeyboardEvent)=>{
...
function updateStick(id:string,patch:Partial<NonNullable<Project['sticks']>[number]>){setP(q=>({...q,sticks:q.sticks.map(s=>{if(s.id!==id)return s;const n={...s,...patch},check=stickCollision(n,q.lines);let nearest=Infinity;for(const l of q.lines)for(const pt of l.points)nearest=Math.min(nearest,Math.hypot(n.x-pt.x,n.y-pt.y));const geometry=Math.max(0,Math.min(100,70-Math.min(45,nearest*120)+Math.min(20,n.length/4)+(n.kind==='avoid'?-20:0)-(check.collision?25:0)));return{...n,score:Math.round(geometry)}}),updatedAt:Date.now()}))}
 ...
 function deleteStick(){if(!selectedStick)return;setP(q=>({...q,sticks:q.sticks.filter(s=>s.id!==selectedStick),history:[...q.history,snapshot('Delete support stick',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}));setSelectedStick(null)}
 ...
 if(cursor==='draw')setP(q=>({...q,lines:[...q.lines,{id:crypto.randomUUID(),points:[pt,{x:pt.x+.04,y:pt.y+.04}],width:q.settings.minWidthMm}],connections:[],history:[...q.history,snapshot('Draw',q.lines,q.sticks,q.productionPlan,q.constraints,q.connections)],updatedAt:Date.now()}))}
