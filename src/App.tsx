...
import type{Line,Project,BatchJob,ConnectionCandidate,CADConstraint}from'./types';
...
export default function App(){
 ...
 useEffect(()=>{listProjects().then(xs=>{const n=xs.map(normalize);setProjects(n);if(n.length)setP(n[0])})},[]); useEffect(()=>{const onKey=(e:KeyboardEvent)=>{
  ...
  if(e.key==='Escape'){setSelectedLine(null);setSelectedLines([]);setRelationLine(null);setSelectedNode(null);setDiagnosticFocus(null);setDiagnosticHover(null)}
 ...
 useEffect(()=>{const c=canvas.current;if(!c)return;const dpr=window.devicePixelRatio||1,w=c.clientWidth,h=c.clientHeight;c.width=w*dpr;c.height=h*dpr;const x=c.getContext('2d')!;x.setTransform(dpr,0,0,dpr,0,0);x.fillStyle=dark?'#101216':'#f4f5f7';x.fillRect(0,0,w,h);x.save();x.translate(w/2+pan.x,h/2+pan.y);x.scale(zoom,zoom);
  ...
  p.lines.forEach(l=>{
    const selected=selectedLines.includes(l.id), issue=canvasLineIssues.get(l.id);
    const issueStroke=issue?.conflict?'#ef5350':issue?.warning?'#f59e0b':undefined;
    drawLine(l,issueStroke??(selected?(dark?'#60a5fa':'#2563eb'):(dark?'#f1f3f5':'#171717')),issue?Math.max(3,l.width*2.2):(selected?Math.max(2.5,l.width*1.8):Math.max(1,l.width*1.5)),viewMode==='original'?.15:1);
    if(issue){
  ...
  if(selectedCad&&selectedCad.points.length>1){
   ...
   const arrow=(px,py,theta)=>{x.beginPath();x.moveTo(px,py);x.lineTo(px-8*Math.cos(theta-.45),py-8*Math.sin(theta-.45));x.lineTo(px-8*Math.cos(theta+.45),py-8*Math.sin(theta+.45));x.closePath();x.fill()};
...
function rebuildTopology(){setP(q=>{const graph=rebuildGraph(q.lines),connections=findConnectionCandidates(q.lines,q.settings.connectDistance),warnings=[...analyze(q.lines),...graph.filter(n=>n.degree>=3).map(n=>({id:crypto.randomUUID(),severity:'low' as const,message:'Junction node detected; review the joint before cooking.',lineIds:n.lineIds}))];return{...q,connections,warnings,sticks:recommendSticks(q.lines),updatedAt:Date.now()}})}
 ...
 function undo(){
  ...
  setP(q=>({...q,lines:cloneLines(h.lines),sticks:h.sticks?.map(s=>({...s}))??q.sticks,constraints:h.constraints?.map(c=>({...c}))??q.constraints,connections:findConnectionCandidates(h.lines,q.settings.connectDistance),warnings:analyze(h.lines),history:q.history.slice(0,-1),updatedAt:Date.now()}));
}
function redo(){
  const h=redoStack.at(-1);if(!h)return;
  historyNavigation.current=true;
  setRedoStack(r=>r.slice(0,-1));
  setP(q=>({...q,lines:cloneLines(h.lines),sticks:h.sticks?.map(s=>({...s}))??q.sticks,constraints:h.constraints?.map(c=>({...c}))??q.constraints,connections:findConnectionCandidates(h.lines,q.settings.connectDistance),warnings:analyze(h.lines),history:[...q.history,snapshot('Undo',q.lines,q.sticks)],updatedAt:Date.now()}));
}
...
function hitNode(pt:{x:number;y:number}){let best:{lineId:string;index:number}|null=null,bd=.028;for(const l of p.lines)for(let i=0;i<l.points.length;i++){const q=l.points[i],d=Math.hypot(q.x-pt.x,q.y-pt.y);if(d<bd){bd=d;best={lineId:l.id,index:i}}}return best}
 ...
 return <div className={dark?'app dark':'app'}><header><div className="brand"><div className="logo">S</div><div><b>SugarDraw</b><small>Buildable sugar art</small></div></div><nav><button>File</button><button>Edit</button><button>View</button><button onClick={undo} disabled={!p.history.length} title="Undo (Ctrl+Z)"><Undo2/></button><button onClick={redo} disabled={!redoStack.length} title="Redo (Ctrl+Y / Ctrl+Shift+Z)"><Redo2/></button><button onClick={()=>downloadSvg(p)}><Download/></button></nav><div className="topActions"><button onClick={()=>setDark(v=>!v)}>{dark?<Sun/>:<Moon/>}</button><button onClick={()=>setBatchOpen(v=>!v)}><Layers/> Batch</button><button className="primary" onClick={()=>document.getElementById('files')?.click()}><Upload/> Import</button><input id="files" hidden type="file" multiple accept=".png,.jpg,.jpeg,image/png,image/jpeg" onChange={e=>importFiles(e.target.files)}/></div></header>
