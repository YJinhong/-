...
export async function opencvSugarMask(file:File,detail:'low'|'balanced'|'high'='balanced'){
 const cv=await loadOpenCV(),{cvMat:src,width:w,height:h}=await imageToMat(file,cv),lab=new cv.Mat(),blur=new cv.Mat(),adaptive=new cv.Mat(),foreground=new cv.Mat(),edges=new cv.Mat(),combined=new cv.Mat();
 try{
  ...
  const k=cv.getStructuringElement(cv.MORPH_ELLIPSE,new cv.Size(params.close,params.close));cv.morphologyEx(foreground,foreground,cv.MORPH_CLOSE,k);cv.morphologyEx(foreground,foreground,cv.MORPH_OPEN,k);k.delete();
